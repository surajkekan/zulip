...
from zerver.models import MultiuseInvite, PreregistrationUser, RealmAuditLog
...
class RevokeInviteTest(ZulipTestCase):
    def test_revoke_invites(self) -> None:
        ...
        prereg_user = PreregistrationUser.objects.create(
            email=email, realm=realm, referred_by=user_profile, status=0
        )
        ...
        multi_invite = MultiuseInvite.objects.create(
            realm=realm, referred_by=user_profile, status=0
        )
        Confirmation.objects.create(
        ...
        found_revoked = any(
            i.get("id") == multi_invite.id and i.get("is_multiuse") for i in invites
        )
        self.assertFalse(
            found_revoked,
            "Revoked multiuse invite should not be returned in controlled invites list",
        )
